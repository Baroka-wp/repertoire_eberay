        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex-1 max-w-2xl mx-2 md:mx-6">
              <div className="relative">
                ...
                <input
                  ...
                  className="block w-full pl-10 pr-12 py-2 md:py-2.5 border border-neutral-300 rounded-lg text-slate-900 placeholder:text-slate-400 bg-white cursor-pointer hover:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-800 focus:border-transparent transition-all text-sm"
                />
                <button
                  type="button"
                  onClick={() => setIsModalOpen(true)}
            ...
            <div className="flex items-center gap-2 md:gap-3">
              ...
              {/* Séparateur */}
              <div className="h-8 w-px bg-neutral-300 hidden md:block"></div>

              {/* Bouton Ajouter (si permissions) */}
              {hasPermission(session?.user?.role, 'create') && (
                <Link
                  href="/ajouter"
                  className="flex items-center gap-2 bg-slate-800 hover:bg-slate-900 text-white px-3 py-2 md:px-4 md:py-2 rounded-lg text-sm font-semibold transition-colors shadow-sm"
                >
                  <Plus size={18} />
